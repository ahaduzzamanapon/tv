...
class ApiService {
  ...
  // ── Build URL with api_key query param ──
  ...
  // ── Generic GET with debug logging ──
  ...
  static Future<Map<String, dynamic>> checkVersion() async {
    debugPrint('[API] Version check → ${AppConfig.versionUrl}');
    try {
      final res = await http
          .get(Uri.parse(AppConfig.versionUrl))
          .timeout(const Duration(seconds: 10));
      debugPrint('[API] Version status: ${res.statusCode}');
      return json.decode(res.body);
    } catch (e) {
      debugPrint('[API] Version check failed: $e');
      rethrow;
    }
  }
